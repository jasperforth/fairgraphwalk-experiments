...
class PokecCombinedReports:
    ...
    def process_experiments_reports_baseline(self, exp_dir: Path, combined_df: pd.DataFrame):
        """
        ...
        if exp_dir.name.startswith("experiment"):
            ...
            def parse_directory_name_baseline(directory_name):
                ...
                sens_attr = '_'.join(parts[parts.index('sens') + 1:parts.index('other')])
                other_attr = '_'.join(parts[parts.index('other') + 1:])
                return p, q, sens_attr, other_attr

            p, q, sens_attr, other_attr = parse_directory_name_baseline(exp_dir.name)
