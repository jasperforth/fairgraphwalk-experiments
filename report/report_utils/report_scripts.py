...
from report_utils import PokecConfusionOld as PokecConfusion, PokecCombinedReports, PokecTSNE

# NOTE PokecConfusionOld fallback, as the new iteration needs debugging for FMMC baseline case. 
...
def get_filtered_attributes(d_experiments: dict,
    ...
    d_graph_attributes = defaultdict(dict)
            ...
            if g_nr in d_experiments[ex]["Graph_nrs"]:
                ...
                if resources_dir.exists():
                    ...
                    if attributes_file.exists():
                        # Read the file to identify label columns dynamically
                        df_attr = pd.read_csv(attributes_file)
                        # Dynamically find label columns
                        label_columns = [col for col in df_attr.columns 
                                         if col.startswith('label_') or 
                                            col.startswith('synth_') or 
                                            '_label_' in col]
                        
                        if not label_columns:
                            print(f"No label columns found in {attributes_file}")
                            continue

                        # Filter the dataframe by user_id and label columns
                        df_attr_filtered = df_attr[["user_id"] + label_columns]
                        d_graph_filtered_attributes[ex][f"Graph_nr_{g_nr}"] = df_attr_filtered
                        d_graph_attributes[ex][f"Graph_nr_{g_nr}"] = label_columns
                    else:
                        print(f"Attributes file does not exist: {attributes_file}")
                else:
                    print(f"Graph dir does not exist: {resources_dir}")
            else:
                print(f"Graph {g_nr} of experiment {ex} not found in the dictionary")

    return d_graph_filtered_attributes, d_graph_attributes
...
def create_reporting_process_dict(data_dir: Path, 
    ...
    experiment_dirs = [ex for ex in data_dir.iterdir() 
                if ex.is_dir() 
                and ex.name.startswith(sub_ex_name_start)
                ]
                ...
                if g_nr in d_experiments[ex]["Graph_nrs"]:
                    ...
                    if graph_dir.exists():
                        exp_graph_name = (f"{ex} {g_nr}")
                         # Use pre-filtered attributes for this graph
                        if f"Graph_nr_{g_nr}" in d_graph_attributes[ex]:
                            attributes = d_graph_attributes[ex][f"Graph_nr_{g_nr}"]  
                        else:
                            print(f"Filtered attributes for Graph {g_nr} of experiment {ex} not found. Skipping...")
                            continue
