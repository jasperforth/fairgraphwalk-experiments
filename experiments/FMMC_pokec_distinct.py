...
def new_run_experiment_pokec_distinct(graphsize: str):
    # Set up directories and seeds for the experiment.
    experiment_name = f"new_pokec_distinct_graphsize_{graphsize}"
    ...
    try:
        ...
        if len(specific_files) < 2:
            ...
            PokecFormatter().pre_formatting(attribute_filepath=RAW_ATTRIBUTES_FILEPATH,
                                            ...
                                            # include_edge_counts=INCLUDE_EDGECOUNTS,
            )
        else:
    ...
    if graphsize == "small":
        region_categories = {
            # 1359 edges 86%
            'graph_dir_0': ['ceska republika, cz - karlovarsky kraj',
                            'ceska republika, cz - kralovehradecky kraj',
                            'ceska republika, cz - budejovicky kraj', ],
            # 9267 edges 93%
            'graph_dir_1': ["bratislavsky kraj, stupava",
            ...
            #9059 edges 82%
            'graph_dir_2': ['ceska republika, cz - praha',
                            'ceska republika, cz - ostravsky kraj',
                            'ceska republika, cz - jihomoravsky kraj',],
            # 3523 edges 83%
            'graph_dir_3': ['bratislavsky kraj, modra',
                            'bratislavsky kraj, bratislava - lamac',
                            'bratislavsky kraj, stupava',
                            'bratislavsky kraj, bratislava - vajnory',
                            'bratislavsky kraj, svaty jur'],
        }
    elif graphsize == "large":
        region_categories = {
            # 36688 edges 93%
            ...
            #78882 edges 95%+
            'graph_dir_1': ['ceska republika, cz - zlinsky kraj',
                            'presovsky kraj, stara lubovna',
                            'zilinsky kraj, kysucke nove mesto'],
            # 60652 edges 98%
            'graph_dir_2': ["presovsky kraj, snina",
                            "trnavsky kraj, holic",
                            "trenciansky kraj, handlova",],
            # 38392 edges 90%
            'graph_dir_3': ["zilinsky kraj, liptovsky hradok",
    ...
    logger.info(f'Running FMMC experiments for all graphs')
        # run FMMC
    try:
        Parallel(n_jobs=WORKERS, verbose=100)(
                delayed(run_fmmc_experiment)(
                            graph=graph, experiment_graph_dir=experiment_graph_dir,
                            p=p, q=q,
                    ...
                    for graph_nr, (graph, experiment_graph_dir) in enumerate(generated_pokec_subgraphs_directories)
                        for selfloops in SELFLOOPS
                            for p in P_VALUES
                                for q in Q_VALUES)
        logger.info(f'Finished FMMC experiments for all graphs')
    except Exception as e:
        logger.error(f"Error running FMMC experiments for all graphs: {e}")
