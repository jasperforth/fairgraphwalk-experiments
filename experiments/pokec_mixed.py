...
def run_experiment_pokec_mixed():
    # Set up directories and seeds for the experiment.
    experiment_name = f"pokec_mixed"
    ...
    def run_baseline_experiment(graph: PokecGraph, 
        ...
        logger.info(f'Running baseline experiment for graph {graph_name} with p={p}, q={q}')
        experiment_graph_dir.mkdir(parents=True, exist_ok=True)
    ...
    def calculate_cfn(graph: PokecGraph,
        ...
        logger.info(f'Calculating CFN for graph {graph_name} with sensitive attribute {sensitive_attribute_name}')
    ...
    def run_crosswalk_experiment(graph: PokecGraph, 
        ...
        logger.info(f'Running CrossWalk experiment for graph {graph_name} with alpha={alpha}, exponent={exponent}, p={p}, q={q}')                    
        experiment_graph_dir.mkdir(parents=True, exist_ok=True)
        result_dir.mkdir(parents=True, exist_ok=True)
        ExperimentRun(graph=graph,
        ...
        try:
            Parallel(n_jobs=WORKERS, verbose=100)(
                    delayed(run_baseline_experiment)(
                        ...
                        for p in P_VALUES
                            for q in Q_VALUES)
            logger.info(f'Finished baseline experiments for graph {graph_nr}')
        except Exception as e:
